// src/components/FloatingLines.jsx
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (enableBottom) {
    for (int i = 0; i < bottomLineCount; ++i) {
      ...
      col += lineCol * wave(ruv + vec2(bottomLineDistance * fi + bottomWavePosition.x, bottomWavePosition.y), 1.5 + 0.3 * fi, baseUv, mouseUv, interactive) * 0.4;
    }
  }
  if (enableMiddle) {
  ...
  if (enableTop) {
    for (int i = 0; i < topLineCount; ++i) {
      ...
      vec2 ruv = baseUv * rotate(angle);
      ruv.x *= -1.0;
      col += lineCol * wave(ruv + vec2(topLineDistance * fi + topWavePosition.x, topWavePosition.y), 1.0 + 0.3 * fi, baseUv, mouseUv, interactive) * 0.4;
    }
  }
  fragColor = vec4(col, 1.8);
}
...
export default function FloatingLines({ 
  ...
  const middleLineDistance = enabledWaves.includes('middle') ? getLineDistance('middle') * 0.02 : 0.02;
